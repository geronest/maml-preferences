...
export WANDB_PROJECT=maml-rlhf-test
...
INNER_BATCH_SIZE=2
GRAD_ACC=2
WANDB_NAME=testmetasft_nolora_fr_lr${LEARNING_RATE}_${NUM_STEPS}steps_batch${BATCH_SIZE}

# sft_output_dir=ckpts/sft_models/${WANDB_NAME}
sft_output_dir=ckpts/test_maml/${WANDB_NAME}
...
BATCH_SIZE=2
GRAD_ACC_STEPS=1
WANDB_NAME=testmetarm_nolora_${NUM_STEPS}steps_lr${LEARNING_RATE}_innerlr${INNER_LR}_gradacc${GRAD_ACC_STEPS}
rm_output_dir=ckpts/test_maml/${WANDB_NAME}
...
# NUM_GENERATIONS=4
# GRAD_ACC_STEPS=4
# WANDB_NAME=testmeta_nolora_${lang}_lr${LEARNING_RATE}_${NUM_STEPS}steps_batch${BATCH_SIZE}_decay${WEIGHT_DECAY}

# reward_path=${REWARD_SEARCH_CKPT}
# sft_path=${SFT_SEARCH_CKPT}

# policy_output_dir=ckpts/test_maml/${WANDB_NAME}
